...
public class SDialogStockCardex extends JDialog implements ActionListener {
    ...
    private void initComponentsExtra() {
        ...
        moGridStockMoves = new SGridPaneForm(miClient, SModConsts.O_STK, SLibConsts.UNDEFINED, "Movimientos inventarios") {
            @Override
            ...
            public ArrayList<SGridColumnForm> createGridColumns() {
                ...
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_DATE, "Fecha"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_TEXT_CODE_CO, "Sucursal"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_TEXT_CODE_CO, "Almacén"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_INT_1B, "# movimiento"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_TEXT_NAME_CAT_S, "Tipo movimiento"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_TEXT_CODE_CAT, "Tipo ajuste"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_DEC_4D, "Entradas"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_DEC_4D, "Salidas"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_DEC_4D, "Existencias"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_TEXT_CODE_UNT, "Unidad"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_TEXT_REG_NUM, "Folio docto inv"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_DATE, "Fecha docto inv"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_TEXT_CODE_CAT, "Tipo docto CV"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_TEXT_REG_NUM, "Folio docto CV"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_DATE, "Fecha docto CV"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_TEXT_CODE_CAT, "Tipo docto CV ajuste"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_TEXT_REG_NUM, "Folio docto CV ajuste"));
                columns.add(new SGridColumnForm(SGridConsts.COL_TYPE_DATE, "Fecha docto CV ajuste"));
    ...
    private void showStockMoves() {
        ...
        try {
            sql = "SELECT s.id_year, s.id_item, s.id_unit, s.id_co, s.id_cob, s.id_wah, s.id_mov, " + // 7
                    ...
                    "adjt.code, CONCAT(adj.num_ser, IF(length(adj.num_ser) = 0, '', '-'), adj.num) AS f_adj_num, adj.dt, adj_cob.code " + // 32
                    "FROM " + SModConsts.TablesMap.get(SModConsts.O_STK) + " AS s " +
                    "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.OS_IOG_TP) + " AS st ON s.fk_iog_ct = st.id_iog_ct AND s.fk_iog_cl = st.id_iog_cl AND s.fk_iog_tp = st.id_iog_tp " +
                    "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.O_IOG) + " AS iog ON s.fk_iog = iog.id_iog " +
                    "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.OS_IOG_TP) + " AS iogt ON iog.fk_iog_ct = iogt.id_iog_ct AND iog.fk_iog_cl = iogt.id_iog_cl AND iog.fk_iog_tp = iogt.id_iog_tp " +
                    "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.OS_ADJ_TP) + " AS ad ON iog.fk_iog_adj_tp = ad.id_iog_adj_tp " +
    ...
    public void setFormParams(final Date dateCutOff, final int itemId, final int unitId, final int[] warehouseKey) {
        ...
        if (itemId == SLibConsts.UNDEFINED) {
            ...
            moTextItemName.setValue(miClient.getSession().readField(SModConsts.OU_ITM, new int[] { itemId }, SDbRegistryUser.FIELD_NAME));
            moTextItemCode.setValue(miClient.getSession().readField(SModConsts.OU_ITM, new int[] { itemId }, SDbRegistryUser.FIELD_CODE));
